...
def create_optimizer_or_freeze_model_dynamic(model, cfg_train, global_step):
    ...
    for k in cfg_train.keys():
        ...
        if not hasattr(model.radiancegrid, k):
            continue

        param = getattr(model.radiancegrid, k)
        if param is None:
    ...
    for k in cfg_train.keys():
        ...
        if not hasattr(model.deformgrid, k):
            continue

        param = getattr(model.deformgrid, k)
        if param is None:
            print(f'create_optimizer_or_freeze_model deformgrid: param {k} not exist')
            continue

        lr = getattr(cfg_train, f'lrate_{k}') * decay_factor
        if lr > 0:
            print(f'create_optimizer_or_freeze_model deformgrid: param {k} lr {lr}')
            if isinstance(param, nn.Module):
                param = param.parameters()
            param_group.append({'params': param, 'lr': lr})
        else:
            print(f'create_optimizer_or_freeze_model deformgrid: param {k} freeze')
            param.requires_grad = False
...
def load_model_dynamic(model_class, ckpt_path):
    ckpt = torch.load(ckpt_path)
    model = model_class[0](
        model_class[1](**ckpt['deform_kwargs']),
        model_class[2](**ckpt['radiance_kwargs'])
        )
    model.load_state_dict(ckpt['model_state_dict'])
    return model
